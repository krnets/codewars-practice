...
public class Game {
    public String winner(String[] deckSteve, String[] deckJosh) {
        var cardRank = "23456789TJQKA";
        int scoreSteve = 0;
        int scoreJosh = 0;

        for (int i = 0; i < deckSteve.length; i++) {
            int flipScore = cardRank.indexOf(deckSteve[i]) - cardRank.indexOf(deckJosh[i]);

            if (flipScore > 0) scoreSteve++;
            if (flipScore < 0) scoreJosh++;
        }
        return scoreSteve > scoreJosh ? String.format("Steve wins %d to %d", scoreSteve, scoreJosh) :
                scoreJosh > scoreSteve ? String.format("Josh wins %d to %d", scoreJosh, scoreSteve) : "Tie";
    }
}