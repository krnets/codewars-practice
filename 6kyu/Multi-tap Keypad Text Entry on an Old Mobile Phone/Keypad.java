...
Prior to the development of T9 (predictive text entry) systems, the method to type words was called
"multi-tap" and involved pressing a button repeatedly to cycle through the possible values.
...
/*
public class Keypad {
    static String[] KEYPAD = {"1", "ABC2", "DEF3", "GHI4", "JKL5", "MNO6", "PQRS7", "TUV8", "WXYZ9", "*", " 0", "#"};

    public static int presses(String phrase) {
        int count = 0;

        for (var c : phrase.toUpperCase().split(""))
            for (var key : KEYPAD)
                if (key.contains(c))
                    count += (key.indexOf(c) + 1);
...
public class Keypad {
    ...
    public static int presses(String phrase) {
        ...
        for (var c : phrase.toUpperCase().toCharArray())
            for (var key : KEYPAD)
                count += (key.indexOf(c) + 1);
...
/*
public class Keypad {
    static List<String> keypad = Arrays
            .asList("1", "ABC2", "DEF3", "GHI4", "JKL5", "MNO6", "PQRS7", "TUV8", "WXYZ9", "*", " 0", "#");

    public static int presses(String phrase) {
        return Arrays.stream(phrase.toUpperCase().split(""))
                .mapToInt(Keypad::keyPressCounter)
                .sum();
    }

    private static int keyPressCounter(String c) {
        return keypad.stream()
                .filter(key -> key.contains(c))
                .mapToInt(key -> key.indexOf(c) + 1)
                .sum();
    }
}
*/
...
public class Keypad {
    static List<String> KEYS = Arrays
            .asList("1", "ABC2", "DEF3", "GHI4", "JKL5", "MNO6", "PQRS7", "TUV8", "WXYZ9", "*", " 0", "#");

    public static int presses(String phrase) {
        return phrase.toUpperCase().chars()
                .flatMap(c -> KEYS.stream().mapToInt(key -> key.indexOf(c) + 1))
                .sum();
    }
}
