...
In esoteric language called Whitespace, numbers are represented in the following format:
        ...
        [space] for plus, [tab] for minus;
        characters after that and until [LF] are the binary representation of the integer:
        [space] for 0, [tab] for 1.
        characters after that are the binary representation of the absolute value of the number,
        with [space] for 0, [tab] for 1, the rightmost bit is the least significand bit,
        and no leading zero bits.
        the binary representation is immediately followed by [LF].

Notes

        Valid Whitespace number must always have at least two characters:
        a sign and the terminator. In case there are only two characters,
        ...
        For your convenience, in this kata we will use unbleach() function when evaluating
        your results. This function replaces whitespace characters with [space], [tab],
        and [LF] to make fail messages more obvious. You can see how it works in Example Test Cases.
...
public class Kata {
    public static String whitespaceNumber(final int n) {
        return (n > 0 ? ' ' : n < 0 ? '\t' : "") + Integer.toBinaryString(Math.abs(n))
                .replace('0', ' ')
                .replace('1', '\t') + '\n';
    }
}