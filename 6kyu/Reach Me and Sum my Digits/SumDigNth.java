...
We define pattern list, patternL, an array that has the differences
...
and are repeated as the differences values of the pattern list establishes.
...
public class SumDigNth {
    public static long sumDigNthTerm(long initval, long[] patternl, int nthterm) {
        Function<long[], Long> sumArray = arr -> Arrays.stream(arr).sum();

        Function<Long, Long> sumDigits = n -> String.valueOf(n).chars()
                .mapToLong(Character::getNumericValue).sum();

        int len = patternl.length;
        int iterations = nthterm - 1;
        int cycles = iterations / len;
        int offset = iterations % len;
        long patternSum = sumArray.apply(patternl);
        long base = patternSum * cycles;

        return sumDigits.apply(initval + base + sumArray.apply(Arrays.copyOf(patternl, offset)));
    }
}
*/
