...
namespace DictionaryMergerKata
{
    ...
    public class AccountTest
    {
        ...
        public void Merge_EmptyDictionaries_Returns_Empty_Dictionary()
        {
            ...
            Assert.IsEmpty(DictionaryMerger.Merge(empty1, empty2),
                "Two empty dictionaries should create an empty dictionary");
            Assert.IsEmpty(DictionaryMerger.Merge(empty1, empty2, empty3),
                "Multiple empty dictionaries should create an empty dictionary");
        }
        ...
        private void Compare<TKey, TValue>(Dictionary<TKey, TValue[]> actual, Dictionary<TKey, TValue[]> expected)
        {
            Assert.AreEqual(expected.Count, actual.Count,
                "The dictionary does not contain the expected number of elements");
            foreach (var actualPair in actual)
            {
                ...
                CollectionAssert.AreEquivalent(expectedValue, actualPair.Value,
                    "The values in the dictionary are not matching");
            }
        }
    }
}