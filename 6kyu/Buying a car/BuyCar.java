...
public class BuyCar {
    public static int[] nbMonths(int startPriceOld, int startPriceNew, int savingperMonth, double percentLossByMonth) {
        int savingsCurrent = 0;
        int monthsToBuildSavings = 0;

        double priceOld = startPriceOld;
        double priceNew = startPriceNew;

        while (priceNew - priceOld > savingsCurrent) {
            if (++monthsToBuildSavings % 2 == 0) {
                percentLossByMonth += 0.5;
            }
            priceOld -= priceOld * (percentLossByMonth / 100);
            priceNew -= priceNew * (percentLossByMonth / 100);

            savingsCurrent += savingperMonth;
        }
        int savingsAfter = (int) Math.round(priceOld - priceNew + savingsCurrent);

        return new int[]{monthsToBuildSavings, savingsAfter};
    }
}
*/
