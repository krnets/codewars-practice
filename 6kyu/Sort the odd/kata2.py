...
#     odd = sorted((x for x in source_array if x & 1), reverse=True)
...
# def sort_array(source_array):
#     odd = sorted((x for x in source_array if x & 1), reverse=True)
#     return [odd.pop() if x & 1 else x for x in source_array]


def sort_array(source_array):
    odds = iter(sorted(x for x in source_array if x & 1))
    return [next(odds) if x & 1 else x for x in source_array]
