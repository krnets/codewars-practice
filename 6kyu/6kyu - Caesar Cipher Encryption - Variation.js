// 6kyu - Caesar Cipher Encryption - Variation
...
The key for this cipher can be any non-negative integer. However, because there are 26 letters in English alphabet, 
any key larger than 26 can be thought about as key = key % 26.
...
However, the key should be increased by 1 with every word in a phrase. For example, if the function is called with a phrase 
...
//             inner.push((arr[i][j].charCodeAt() - 97 + shift + i) % 26 + 97)
//         res.push(inner.map(v => String.fromCharCode(v)).join(''))
//     }
//     return res.join(' ')
// }

const caesarEncode = (phrase, shift) => phrase.split(' ').map((v, i) => [...v].map(x => String.fromCharCode((x.charCodeAt() - 97 + shift + i) % 26 + 97)).join('')).join(' ')

q = caesarEncode("alea iacta est", 3)
//  "dohd megxe jxy")
q
q = caesarEncode("conquer et impera", 13)
//  "pbadhre sh xbetgp"
q
q = caesarEncode("fere libenter homines id quod volunt credunt", 7)
// "mlyl tqjmvbmz qxvrwnb sn bfzo haxgzf perqhag"
q
q = caesarEncode("horum omnium fortissimi sunt belgae", 0)
