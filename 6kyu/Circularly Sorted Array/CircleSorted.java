...
public class CircleSorted {
    public boolean isCircleSorted(int[] a) {
        int[] aCloneSorted = a.clone();
        Arrays.sort(aCloneSorted);

        for (int i = 1; i <= a.length; i++) {
            int[] aRotated = new int[a.length];

            System.arraycopy(a, 0, aRotated, a.length - i, i);
            System.arraycopy(a, i, aRotated, 0, a.length - i);

            if (Arrays.equals(aCloneSorted, aRotated))
                return true;
        }
        return false;
    }
