/*
You know combinations:
for example, if you take 5 cards from a 52 cards deck
...
The formula to compute m = n choose x is:
...
Fortunately if you take 7 choose 3 or 7 choose 4 combinations you get 35
and so each poster will have a different combination of 3 colors or 5 colors.
...
Hence the problem is:
        ...
        let us search x (number of colors for each poster
        so that each poster has a unique combination of colors
        and the number of combinations is exactly the same as the number of posters).

        In other words we must find x such as n choose x = m (1)
        for a given m and a given n; m >= 0 and n > 0.
        If many x are solutions give as result the smallest x.
        It can happen that when m is given at random there are no x
        satisfying equation (1) then return -1.
...
public class Checkchoose {
    public static long checkchoose(long m, int n) {
        ...
        return LongStream.iterate(0, i -> i < n, i -> i + 1)
                .filter(i -> colors.getAndSet(colors.get() * (n - i) / (i + 1)) == m)
                .findFirst()
                .orElse(-1);
    }
}
