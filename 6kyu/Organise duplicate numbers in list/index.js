...
// function group(arr) {
//     let countNums = {};
//     arr.forEach(v => countNums[v] = ++countNums[v] || 1)
//     return Array(...new Set(arr)).map(v => (String(v) + '!').repeat(countNums[v]).split('!').filter(v => v).map(Number))
// }

const group = arr => [...new Set(arr)].map(n => arr.filter(x => x == n))


q = group([3, 2, 6, 2, 1, 3]) // [[3, 3], [2, 2], [6], [1]]
q
q = group([3, 2, 6, 2]) // [[3], [2, 2], [6]]
q