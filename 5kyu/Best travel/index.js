...
The biggest possible sum taking a limit of 174 into account is then 173 and the distances of the 
3 corresponding towns is [55, 58, 60].
...
const chooseBestSum = (t, k, ls) =>
    ls.reduce((arr, v) => arr
        .concat(arr.filter(c => c.length < k)
            .map(c => c.concat(v))), [
            []
        ])
    .filter(v => v.length == k)
...
ts = [50, 55, 56, 57, 58]
q = chooseBestSum(163, 3, ts) // 163
q
ts = [50]
q = chooseBestSum(163, 3, ts) // null
q
ts = [91, 74, 73, 85, 73, 81, 87]
q = chooseBestSum(230, 3, ts) // 228
q