const changeCase = (identifier, targetCase) => {
    if (!/^(snake|camel|kebab)$/.test(targetCase)) return undefined;
    if (/(-.*[_A-Z])|(_.*[-A-Z])|([A-Z].*[_-])/.test(identifier)) return undefined;
   
   let output = '';
   switch (targetCase) {
     case 'snake':
       output = identifier.replace(/[A-Z-]/g, char => (char === '-' ? '_' : '_' + char.toLowerCase()) );
       break;
     case 'camel':
       output = identifier.replace(/[_-][a-z]/g, char => char[1].toUpperCase() );
       break;
     case 'kebab':
       output = identifier.replace(/[A-Z_]/g, char => (char === '_' ? '-' : '-' + char.toLowerCase()) );
       break;
   }
