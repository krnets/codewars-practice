...
#     return re.sub(r'(and|but\b)(?!\s+apparently\b)', lambda m: m.group(0) + ' apparently', string)

def apparently(string):
    return re.sub(r'(and|but\b)(?!\s+apparently\b)', r'\1 apparently', string)
...
q = apparently('and')  # 'and apparently'
q
q = apparently('apparently')  # 'apparently'
q
