...
Transform the given string (without spaces) to a backronym, using the preloaded dictionary
and return a string of words, separated with a single space (but no trailing spaces).
...
public class Backronym {
    ...
    public static String makeBackronym(String acronym) {
        return Stream.of(acronym.toUpperCase().split(""))
                .map(c -> dictionary.getOrDefault(c, c))
...
public class Backronym {
    ...
    public static String makeBackronym(String acronym) {
        return acronym.toUpperCase().chars()
                .mapToObj(i -> dictionary.get("" + (char) i))
                .collect(Collectors.joining(" "));
    }
}
