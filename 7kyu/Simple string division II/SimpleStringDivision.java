...
Lets take pairs of these numbers, concatenate each pair
and determine how many of them of divisible by k.

        If k = 3,
        we get following numbers ['12', '18', '21', '24', '42', '48', '81', '84'],
        all divisible by 3.
...
return the number of pairs that when concatenated, are divisible by k.
    ...
    public static int solve(String s, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            for (int j = 0; j < nums.length; j++) {
                if (i == j)
                    continue;
                if (Integer.parseInt(nums[i] + nums[j]) % k == 0)
                    count++;
            }
        }
        return count;
