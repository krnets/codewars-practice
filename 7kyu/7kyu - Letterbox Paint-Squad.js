...
numbers on people's letterboxes for a small fee.
...
function paintLetterboxes(start, end) {
    let obj = {}
    Array(10).fill().map((_, i) => obj[i] = 0)
    Array(end - start + 1).fill().map((_, i) => i + start).join('').split('').map(v => ++obj[v])
    return Object.values(obj)
}


q = paintLetterboxes(125, 132) // [1,9,6,3,0,1,1,1,1,1]
q