...
The original number is divisible by 7 if and only if the last number obtained using this procedure
is divisible by 7.
        ...
        Your task is to return to the function seven(m) (m integer >= 0) an array of numbers,
        the first being the last number m with at most 2 digits obtained by your function
        (this last m will be divisible or not by 7),
        the second one being the number of steps to get the result.

        Return { number-of-steps, last-number-m-with-at-most-2-digits }
...
public class DivSeven {
    public static long[] seven(long m) {
        int steps = 0;
        while (Integer.toString((int) m).length() > 2) {
            m = m / 10 - m % 10 * 2;
            steps++;
        }
