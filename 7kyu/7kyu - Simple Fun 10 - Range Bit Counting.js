...
// const rangeBitCount = (a, b) => [...Array.from({length: b - a + 1}, (_, i) => a + i).map(digit => digit.toString(2)).join ``].filter(bit => bit == 1).length
// const rangeBitCount = (a, b) => [...Array.from({length: b - a + 1}, (_, i) => (a + i).toString(2)).join``].filter(bit => bit==1).length
const rangeBitCount = (a, b) => Array.from({length: b - a + 1}, (_, i) => (a + i).toString(2).replace(/0/g,``)).join``.length


q = rangeBitCount(2, 7) // 11
q
q = rangeBitCount(0, 1) // 1
q
q = rangeBitCount(4, 4) // 1
q