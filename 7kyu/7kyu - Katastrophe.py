...
A building begins with a strength value of 1000 when first built, but this value is subject 
to exponential decay of 1% per year. For more info on exponential decay, follow this link - 
...
#     magnitude = reduce(lambda x, y: x * y, [sum(x) for x in earthquake])
#     strength = 1000 * pow(0.99, age)
#     return 'Safe!' if strength > magnitude else 'Needs Reinforcement!'

def strong_enough(earthquake, age):
    magnitude = reduce(lambda x, y: x * y, [sum(x) for x in earthquake])
    strength = 1000 * pow(0.99, age)
    return 'Needs Reinforcement!' if strength < magnitude else 'Safe!'


q = strong_enough([[2, 3, 1], [3, 1, 1], [1, 1, 2]], 2)  # "Safe!"
q
q = strong_enough([[5, 8, 7], [3, 3, 1], [4, 1, 2]], 2)  # "Safe!"
q
q = strong_enough([[5, 8, 7], [3, 3, 1], [4, 1, 2]],
