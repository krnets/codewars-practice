...
article to the editor. The article is not complete without this word. Maya has a friend, Dan, and he is very good with words, 
...
const hiddenWord = (num) => num ? (s = [...String(num)], "oblietadnm"[s[0]] + hiddenWord(s.slice(1).join(''))) : ''
// const hiddenWord = (num) => [...String(num)].map(v => "oblietadnm"[v]).join('')
// const hiddenWord = (num) => String(num).replace(/./g, n => "oblietadnm"[n])
// const hiddenWord = (num) => `${num}`.replace(/\d/g, n => "oblietadnm"[n])

q = hiddenWord(637) // "aid"
q
q = hiddenWord(7415) // "debt"
q
q = hiddenWord(49632) // "email"
q
q = hiddenWord(942547) // "melted"
q