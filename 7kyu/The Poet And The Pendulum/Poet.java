...
Notes
        ...
        In Even array size,
        The minimum element should be moved to (n-1)/2 index (considering that indexes start from 0)
...
Input >> Output Examples:
        ...
        Remeber , In Even array size , The minimum element moved to (n-1)/2 index
...
public class Poet {
    public static int[] pendulum(final int[] values) {
        ...
        var position = (values.length - 1) / 2;
        Arrays.sort(values);
        for (int i = 0; i < values.length; i++) {
            if (i % 2 == 0)
                position -= i;
