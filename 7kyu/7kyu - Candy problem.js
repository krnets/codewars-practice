...
Your job is to find out how much candy each child has, and give them each additional candy until they too have as much 
as the child(ren) with the most candy. You also want to keep a total of how much candy you've handed out because reasons."
...
function candies(kids) {
    ...
    return kids.length > 1 ? kids.map(v => max - v).reduce((a, b) => a + b, 0) : -1
}

q = candies([5, 8, 6, 4]) // 9
q
q = candies([1, 2, 4, 6]) // 11
q
q = candies([1, 2]) // 1
q
q = candies([4, 2]) // 2
