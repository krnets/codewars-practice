...
The idea is that if the packet is changed, the checksum will also change, so checksums
are often used for detecting transmission errors, validating document contents,
...
times the character’s value. A space has a value of zero, while letters have a value
equal to their position in the alphabet.

So, A = 1, B = 2, etc., through Z = 26.
Here are example Quicksum calculations for the packets “ACM” and “A C M”:
...
When the packet doesn't have only uppercase letters and spaces or just spaces
the result to quicksum have to be zero (0).
...
public class Quicksum {
    public int quicksum(String packet) {
        return !packet.matches("[A-Z\\s]+") ? 0 :
                IntStream.range(0, packet.length())
                        .map(i -> packet.charAt(i) == 32 ? 0 : (packet.charAt(i++) - 64) * i)
                        .sum();
    }
}
*/
...
public class Quicksum {
    public int quicksum(String packet) {
        return Pattern.compile("[^(A-Z\\s)]").matcher(packet).find() ? 0 :
                IntStream.range(0, packet.length())
                        .map(i -> packet.charAt(i) == 32 ? 0 : (packet.charAt(i++) - 64) * i)
                        .sum();
    }
