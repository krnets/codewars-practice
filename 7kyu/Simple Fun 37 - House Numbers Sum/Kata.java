...
To make the walk more fun he decides to add up all the numbers
of the houses that he passes by during his walk.
...
public class Kata {
    public static int houseNumbersSum(final int[] arr) {
        return IntStream.of(arr).takeWhile(v -> v != 0).sum();
    }
}