...
public class Solution {
    public static List arrayLeaders(int[] numbers) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            if (numbers[i] > IntStream.of(numbers).skip(i + 1).sum())
...
public class Solution {
    public static List arrayLeaders(int[] numbers) {
        var result = new ArrayList<Integer>();
        for (int i = numbers.length - 1, sum = 0; i > -1; i--) {
            if (numbers[i] > sum)
                result.add(0, numbers[i]);
            sum += numbers[i];
        }
        return result;
    }
...
public class Solution {
    public static List arrayLeaders(int[] numbers) {
        return range(0, numbers.length)
                .filter(i -> numbers[i] > of(numbers).skip(i + 1).sum())
//                .map(i -> numbers[i]).boxed()
