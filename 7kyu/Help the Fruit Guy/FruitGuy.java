...
Your task is to implement a method that accepts an array of strings containing fruits should return
an array of strings where all the rotten fruits are replaced by good ones.

Notes : If the array is null or empty you should return empty array ([]).
...
public class FruitGuy {
    public static String[] removeRotten(String[] fruitBasket) {
        return fruitBasket == null ? new String[0] : Arrays.stream(fruitBasket)
                .map(fruit -> fruit.replace("rotten", "").toLowerCase())
                .toArray(String[]::new);
    }
}