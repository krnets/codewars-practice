...
1 bits from their binary representation, while the even numbers will fight using their 0 bits. 
...
function bitsBattle(numbers) {
    let even = numbers.filter(v => v % 2 == 0).map(v => v.toString(2)).join('').replace(/1/g, '').length
    let odd = numbers.filter(v => v % 2 != 0).map(v => v.toString(2)).join('').replace(/0/g, '').length
    return even > odd ? 'evens win' : even < odd ? 'odds win' : 'tie'
}

q = bitsBattle([5, 3, 14]) // 'odds win'
q
q = bitsBattle([3, 8, 22, 15, 78]) // 'evens win'
q
q = bitsBattle([]) // 'tie'
q
q = bitsBattle([1, 13, 16]) // 'tie'
q