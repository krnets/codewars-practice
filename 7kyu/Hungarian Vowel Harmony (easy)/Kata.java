...
Task:   Your goal is to create a function dative() which returns the valid form
        of a valid Hungarian word w in dative case i. e. append the correct suffix nek or nak
...
public class Kata {
    public static String dative(String word) {
        String s = word.replaceAll("[^aáoóuúeéiíöőüű]", "");
        return s.substring(s.length() - 1).matches("[eéiíöőüű]") ? word + "nek" :
               s.substring(s.length() - 1).matches("[aáoóuú]") ? word + "nak" : word;
    }
}
