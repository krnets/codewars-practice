...
4 as after rejecting non integers array will be [1,3,3,4,5] Largest no. is 5. and 2nd largest is 4.

Return nil if there is no 2nd largest integer. Take care of big numbers as well"""


def find_2nd_largest(arr):
    res = sorted(set(filter(lambda x: isinstance(x, int), arr)))
    return res[-2] if len(res) > 1 else None


q = find_2nd_largest([1, 2, 3]), 2
q
q = find_2nd_largest([1, 1, 1, 1, 1, 1, 1]), None
q
q = find_2nd_largest([1, 'a', '2', 3, 3, 4, 5, 'b']), 4
q
q = find_2nd_largest([1, 'a', '2', 3, 3, 3333333333333333333334, 544444444444444444444444444444, 'b']), 3333333333333333333334
q
